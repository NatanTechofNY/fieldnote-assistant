...
export function MemoriesPage() {
  ...
  return <div className="page">
    ...
    {isLoading ? <Loading/> : error ? <ErrorState error={error}/> : kind === "journal" ? <div className="journal">{journalGroups.map(([date, entries]) => <section className="journal-day" key={date}><div className="journal-date">{date}</div><div>{entries.map(memory => <article className="journal-entry" key={memory.id} onDoubleClick={() => setEditor(memory)}><h3><button type="button" className="card-open" onClick={() => setEditor(memory)}>{memory.mood_score ? moodEmoji(memory.mood_score) : "·"} {memory.title || "Untitled entry"}</button><AttachButton item={memoryAttachment(memory)}/></h3><MarkdownContent content={memory.content}/><div className="todo-meta"><MoodPill label={memory.mood_label} moods={memory.moods}/><LifeAreaPill name={memory.life_area_name} slug={memory.life_area_slug}/>{memory.tags.join(" · ")}</div></article>)}</div></section>)}</div> :
