...
The runner also places a progress tapback of its own while the turn's working tools run — 📋, 🧠, ⏰, 📅, 💬, 🗂️, 🪞, 🎫, 📄, or 🛒 for the store the round is reading, 🔍 when it spans several — and lifts it before the reply or before `react_to_message` lands; see [`SMS_AND_EVENTS.md`](SMS_AND_EVENTS.md#reactions-and-threads). It is not a tool and the model is told not to send any of those marks itself.
