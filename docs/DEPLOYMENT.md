...
The `Dockerfile` is multi-stage on `node:22-bookworm-slim`: the build stage installs `python3`, `make`, and `g++` to compile `better-sqlite3`, runs `npm ci` and `npm run build`, then prunes dev dependencies; the runtime stage copies `dist/`, `server/`, `agent-studio/`, and the tsconfigs, exposes `4174`, and starts the server with `node --import tsx server/index.ts`. That is the same process `npm start` runs, invoked directly so that the platform's `SIGTERM` reaches the shutdown handler instead of being absorbed by npm.

The image intentionally has no `VOLUME` instruction. Railway's builder rejects Dockerfiles that contain one (`docker VOLUME is not supported, use Railway Volumes`), so the `/data` requirement is enforced by `requiredMountPath` in `railway.json` on Railway and by the `-v` flag on any other Docker host.
