...
For a message that does get an answer, the worker also sends an indicator explicitly through `/api/send-typing-indicator` before it starts the agent turn, because the account setting is not enough on its own: it lasts 60 seconds and a slow turn outlasts it, so the explicit one asks for 120. The indicator is requested at 0, 8, and 20 seconds into the turn.

The bubble is released as soon as the reply is on its way, since the message itself takes it down, and every attempt still outstanding is dropped with it — otherwise a turn that finished in ten seconds would raise a bubble the reply had already answered. That is also what keeps a typical four-second turn down to a single call. A turn that fails takes the bubble down explicitly with `state: "stop"`.

### The first message after an idle gap gets no bubble

This is a Sendblue limitation and not something the app can work around, so it is worth knowing before it looks like a bug. Sendblue delivers an indicator over the conversation's established route mapping, and after a few minutes of quiet that mapping is cold. A cold indicator is dropped and reported as `SENT` regardless, so nothing on this side can detect it.

Measured against a live account: a thread idle for 5m55s and again for 6m40s got a read receipt and no bubble, with every attempt accepted and reported sent, while the next message 48 seconds after a reply got one. What warms the route is traffic on it, and on the first turn the first thing to travel is the reply itself — which is why retrying inside the turn does not help and the later attempts exist only for a turn slow enough that the route may come back mid-flight. Read receipts are unaffected, because `auto-mark-read` does not depend on that mapping, which is exactly why the gap looks so odd from the phone: the message is visibly read, and then nothing happens until the answer lands.

None of it can fail a turn, and none of the calls are waited for. A refusal does not raise an HTTP error, though: like `send-message`, this endpoint answers `200` with `status: "ERROR"` and a reason in the body. So the body is read, and every answer that is not `SENT` — a refusal, or a shape this app does not recognise — is logged with `console.warn`. Refusals stay out of `integration_settings.last_error` and off the Settings card, because the bubble is decoration and the commonest refusal is a recipient who is on SMS rather than iMessage.

**Saying nothing therefore means Sendblue accepted the request, and nothing else.** That is the one fact worth having when a bubble does not appear: a silent log narrows a missing bubble down to Sendblue dropping an indicator it said it had sent, which is not something this side can see or fix.
