...
Once the agent has reacted — in this attempt, or in an earlier one per the archive — the runtime places nothing further for the rest of the turn, neither progress nor closing: putting a mark up would replace the agent's reaction, and taking it down would leave the message bare. Both marks are the runtime's gestures rather than the model's: they cost no completion, need no prompt compliance, and leave no tool row. A turn that *fails* leaves the progress mark up on purpose: every failed inbound turn is retried behind a short backoff, so the work is still in progress, and the retry reads the archive to see the mark is already there rather than sending it again — one mark for the whole turn however many attempts it takes, replaced by the closing mark from the attempt that answers. Every send is best effort; a reaction Sendblue refuses is logged and the turn continues.
...
- **Inbound events** that throw are marked failed and retried with backoff of `min(3600, 2 ** min(attempts, 10))` seconds, up to `MAX_EVENT_ATTEMPTS` (8, about eight and a half minutes end to end). The attempt that uses the last try settles the event as `ignored` — the same terminal status a dropped echo gets — with `last_error` prefixed `Gave up after 8 attempts:`, lifts the progress tapback the runner left up for a retry that is no longer coming, and texts one line into the same thread (`GAVE_UP_TEXT`, filed as an `assistant` row with `kind: "gave_up"`) so the sender is not left with a 🔍 and silence. Claims left `processing` by a crash are reclaimed after ten minutes. Settled events (`processed` or `ignored`) are pruned by the maintenance pass after seven days; anything still waiting or retrying stays whatever its age. A retry **resumes** the turn rather than restarting it: the inbound row is reused, and `threadHistory()` places the successful write results from the earlier attempt's tool rows after the inbound message as the assistant message that never got written, so a turn that timed out after its `update_todo` does not update again on the retry and then describe the change as pre-existing. Reads are not replayed; repeating a search is cheap. The same replay applies to *every* unanswered user message in the window, not only the one being retried: a later text in the thread reads what the failed turn already wrote instead of finding the request untouched.
