...
`group_id`, `group_name`, and `speaker_name` exist only on messages from a group chat; a 1:1 or web message has the first seven fields and nothing else. `speaker_name` is the name from the trusted-contacts list, never a phone number, and `group_id` is a `filterOnly` facet. When a turn is answered inside a group, the server also sends `algolia.searchParameters` with the completion so the hosted search tool sees only that group's records (see [Group chats](SMS_AND_EVENTS.md#group-chats)); nothing in the dashboard configuration has to change for that. What does have to change when this lands, all under Settings > Under the hood: **Configure Algolia** (or `npm run setup:algolia`), because the messages index gained searchable and facet attributes and settings are not pushed by a reindex; then **Reindex**, so the messages already in SQLite are rebuilt with `group_id`, `group_name`, and `speaker_name`; then **Sync Agent config**, because the client tools, the search tool, and the system prompt changed. A deployment that ships only the code has a model with no `send_message` or `name_group_chat` and name searches that do not match.
...
9. On a todo that already has a reminder, say “remind me at 7 as well.” It should call `create_reminder` with `slot` `extra` and leave the existing reminder in place. A `primary` call here silently overwrites `reminder_at`, and the reminder the user was counting on never fires — the failure this checks for.
10. Reply to a reminder with a new time rather than “done”, such as “I'll go tonight around 7.” It should read that as a reschedule of the todo the reminder named, call `update_todo` to move `due_at`, and repeat the resolved time. Agreeing warmly without a write is the failure this checks for.
11. Tell the agent a field it did not change already holds a value, such as “technically that's due at 7 then.” It must treat the correction as a change request and write it, never confirm the user's version of a record it has not updated.
