...
**If you get `Unreachable hosts - your application id may be incorrect` from a `customGet` or `customPut`,** check the `path` for a leading slash before you check your credentials. Use `1/indexes/...`, not `/1/indexes/...`. The credentials are almost certainly fine, and this message has that one cause: a neural `429` is *not* reported this way, because the client does not retry 4xx responses and passes the rate-limit message straight through.

**If you get `429` with a message about neural index level operations,** you have hit the undocumented cap of ten neural operations per hour per application. Wait for the window; `npm run setup:algolia` skips indices already in the requested state, so it is free to re-run.

**If "Configure Algolia" or `npm run setup:algolia` seems to hang,** it is probably not hanging. Applying settings to a NeuralSearch-active index leaves the task `notPublished` while the index re-vectorizes, which took 269 seconds on six records here. `setup()` no longer waits for publication, so it should return immediately; any tooling of your own that calls `waitForTask` will wait, and may exhaust its 100-poll budget and report a timeout on settings that were accepted in under a second.
